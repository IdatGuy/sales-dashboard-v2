...
export const goalDefinitionsService = {
  async listGoalDefinitions(): Promise<GoalDefinition[]> {
    ...
    if (error) {
      logger.error('Error fetching goal definitions:', error);
      return [];
    }
    return (data || []).map(transform);
  },

  async createGoalDefinition(
    name: string,
    metricKeys: string[],
    unitType: 'currency' | 'count' | 'percentage'
    ...
    if (error) {
      logger.error('Error creating goal definition:', error);
  ...
  async updateGoalDefinition(
    ...
    if (error) {
      logger.error('Error updating goal definition:', error);
      return { success: false, error: 'Failed to update goal. Please try again.' };
    }
    return { success: true };
  },

  async deleteGoalDefinition(
    id: string
  ): Promise<{ success: boolean; error?: string }> {
    const { error } = await supabase
      .from('goal_definitions')
    ...
    if (error) {
      logger.error('Error deleting goal definition:', error);
      return { success: false, error: 'Failed to delete goal. Please try again.' };
    }
    return { success: true };
  },

  async reorderGoalDefinitions(
    orderedIds: string[]
  ): Promise<{ success: boolean; error?: string }> {
    ...
    if (failed?.error) {
      logger.error('Error reordering goal definitions:', failed.error);
