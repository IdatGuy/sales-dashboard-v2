...
export const salesService = {
  async upsertDailySales(
    ...
    if (error) {
      logger.error('Error upserting sales:', error);
      return { success: false, error: 'Failed to save sales data. Please try again.' };
    }
    return { success: true };
  },

  async getStoreDailySales(storeId: string, month: string): Promise<UISale[]> {
  ...
  async getStoreMonthlySales(storeId: string, year: string): Promise<UISale[]> {
    if (!/^\d{4}$/.test(year)) {
    ...
    const { data, error } = await supabase
      ...
      .lte('date', end);
    if (error) {
      logger.error('Error fetching monthly sales:', error);
      return [];
    }
    return (data || []).map(transformDbSaleToSale);
  },

  async bulkUpsertSalesFromCsv(
